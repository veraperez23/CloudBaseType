...
class CloudDataset(BaseDataset):
    # Añadimos 'transform' a los argumentos
    def __init__(self, folder_path, txt_path, transform=None):
        ...
        # 2. OPTIMIZACIÓN: Leemos el .txt UNA SOLA VEZ y lo guardamos en un "diccionario"
        self.labels = {}
            ...
            for line in f:
                ...
                if len(partes) >= 2:
                    try:
                        ...
                        # Solo si es un número, verificamos que la imagen exista y la añadimos
    ...
    def __getitem__(self, index):
        ...
        # Abrimos la imagen en formato PIL (como lo requieren tus augmentations)
        ...
        if self.transform:
            ...
            # Si no hay transform, lo pasamos a tensor manualmente
            image = np.array(image) / 255.0
            image = image.astype(np.float32)
            image = np.transpose(image, (2, 0, 1))
            image = torch.from_numpy(image)

        # 4. Buscamos la altura en nuestro diccionario 
